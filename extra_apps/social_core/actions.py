...
def do_complete(backend, login, user=None, redirect_name='next',
    ...
    responce = backend.strategy.redirect(url)
    responce.set_cookie("name", user.name if user.name else user.username,max_age = 3600)
    responce.set_cookie("token", jwt_encode_handler(payload))
    return responce
